...
MeshCylinder::MeshCylinder(Shader * shader, float radius, float height, UINT stackCount, UINT sliceCount)
	: Mesh(shader), topRadius(radius), botRadius(radius), height(height), stackCount(stackCount), sliceCount(sliceCount)
{

}

MeshCylinder::MeshCylinder(Shader * shader, float topRadius, float botRadius, float height, UINT stackCount, UINT sliceCount)
	: Mesh(shader), topRadius(topRadius), botRadius(botRadius), height(height), stackCount(stackCount), sliceCount(sliceCount)
{
...
void MeshCylinder::Create()
{
	...
	for (UINT i = 0; i < ringCount; i++)
	{
		...
		float r = i * radiusStep	+ botRadius;
...
void MeshCylinder::BuildBottomCap(vector<MeshVertex>& vertices, vector<UINT>& indices)
{
	...
	for (UINT i = 0; i <= sliceCount; i++)
	{
		float x = botRadius * cosf(i * theta);
		float z = botRadius * sinf(i * theta);
